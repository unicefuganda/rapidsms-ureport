...
var pie_opts={chart:{renderTo:"pie",margin:[5,5,5,5]},title:{text:""},plotArea:{shadow:!0,borderWidth:30,backgroundColor:null},tooltip:{formatter:function(){return"<b>"+this.point.name+"</b>: "+this.y.toFixed(1)+" %"}},plotOptions:{pie:{allowPointSelect:!0,cursor:"pointer",dataLabels:{enabled:!0,formatter:function(){return"uncategorized"==this.point.name?"N/A":this.point.name},color:"white",style:{font:"13px Trebuchet MS, Verdana, sans-serif"}}}},legend:{layout:"horizontal",style:{left:"auto",bottom:"auto",
...
function action(a,b,c){if(!$(a).is(".delete")||confirm("Are you sure?"))$("#input_action").val(b),form=$(a).parents("form"),form_data=form.serializeArray(),form_data.concat([{page_num:c}]),ajax_loading(a),$("#object_list").load("./",form_data,function(){$(".ajax_loading").remove()})}
function actionx(a,b,c){if(!$(a).is(".delete")||confirm("Are you sure?"))$("#input_action").val(b),form=$(a).parents("form"),form_data=form.serializeArray(),form_data.concat([{page_num:c}]),ajax_loading(a),$("#object_list").load("./",form_data,function(){$(".ajax_loading").remove()})}
