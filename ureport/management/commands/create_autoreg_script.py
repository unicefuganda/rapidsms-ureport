...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        poll4 = Poll.create_custom('district',"contactdistrict", "Its important to know which District you'll be reporting on so we can work together to try & resolve issues in your community!Reply ONLY with your district.", "Thank you for sending us your district.", [], user)
        script.steps.add(ScriptStep.objects.create(
            script=script,
            poll=poll4,
            order=3,
        ...
        script.steps.add(ScriptStep.objects.create(
            script=script,
            message="You will receive a weekly sms question,for example:'Is there a safe water source within a 10 minute walk from your home?Yes or No' U will b able 2 reply 4 FREE!",
            order=5,
            rule=ScriptStep.WAIT_MOVEON,
            start_offset=0,
            giveup_offset=120,
        ...
        script.steps.add(ScriptStep.objects.create(
            script=script,
            message="Ureport wants to know more about our reporters.UNICEF is excited you've joined Ureport & thank you for answering the questions.This system is FREE!",
            order=6,
            rule=ScriptStep.WAIT_MOVEON,
            start_offset=0,
            giveup_offset=120,
        ))
        poll2 = Poll.create_freeform("contactname", "Best responses may be shared!Send us your name,or if you prefer a nickname so we can give u recognition for good replies by sharing them in the media!", "Thank you for sending us your name, the update has been successful!", [], user)
        script.steps.add(ScriptStep.objects.create(
            script=script,
            poll=poll2,
            order=7,
        ...
        poll5 = Poll.create_numeric("contactage", "What is your age?", "Thank you for sending us your age.", [], user)
        script.steps.add(ScriptStep.objects.create(
            script=script,
            poll=poll5,
            order=8,
        ...
        poll3 = Poll.create_freeform("contactgender", "Are you male or female?  Type F for female, and M for Male", "Thank you for sending us your gender, your update has been successful.", [], user)
        script.steps.add(ScriptStep.objects.create(
            script=script,
            poll=poll3,
            order=9,
        ...
        script.steps.add(ScriptStep.objects.create(
            script=script,
            message="CONGRATULATIONS!!! You are now a registered member of Ureport! With Ureport, you can make a real difference!  Speak Up and Be Heard!",
            order=11,
            rule=ScriptStep.WAIT_MOVEON,
            start_offset=120,
            giveup_offset=86400,
